...
static Rain1Controller *me;
@implementation Rain1Controller
...
-(void)viewWillAppear:(BOOL)animated{
	self.contentSizeForViewInPopover = CGSizeMake(285, 368);
}
...
//		NSLog(@"Rain1Controller*************%d",temPage);
		if (nil == ((RainTableController *)[AllPages objectAtIndex:temPage]).view.superview) {
...
-(void)reloadData{
	...
	numPages=(countrain%NUMPAGE==0?countrain/NUMPAGE:countrain/NUMPAGE+1);
	[self loadPages];
}
...
//		NSLog(@"Rain1Controller ------- %@",[[self.hisc objectAtIndex:1] objectAtIndex:0]);
	...
	{
		RainTableController *viewController = [AllPages objectAtIndex:i];
...
- (void)loadScrollViewWithPage:(int)temPage {
	...
	if([AllPages count]!=0){
		RainTableController *controller = [AllPages objectAtIndex:temPage];
		if((NSNull *)controller == [NSNull null]){
			RainTableController *controller = [RainTableController alloc];
			controller.navagController=self.navigationController;
			controller=[controller initWithPageNumber:temPage withTotalArray:self.hisc];
		...
		if (nil == ((RainTableController *)[AllPages objectAtIndex:temPage]).view.superview) {
			...
			frame.origin.y = 0;
			((RainTableController *)[AllPages objectAtIndex:temPage]).view.frame = frame;
			[myScroll addSubview:((RainTableController *)[AllPages objectAtIndex:temPage]).view];
			
			if(numPages > 0)
			{
				[((RainTableController *)[AllPages objectAtIndex:temPage]) getDataOnNewThread];
			}
		}
	}
}
