...
@implementation OneRainView
@synthesize delegate,badgeNumber;
-(id)initWithFrame:(CGRect)frame WithImage:(UIImage *)imageType convertData:(NSString *)info{
	if (self = [super initWithFrame:frame])
	{
		...
		if (info!=nil&&[info intValue]>0){
			[myButton addTarget:self action:@selector(drawRainPointAndSPopver:) forControlEvents:UIControlEventTouchUpInside];
		...
		if (info!=nil&&[info intValue]>0) {
			...
			TDBadgeView *myBadge = [[TDBadgeView alloc] initWithFrame:CGRectZero myBadgeNumber:info];
...
-(IBAction)drawRainPointAndSPopver:(id)sender{
	[delegate dealWithRainPacHour:self convertRainType:self.tag];
}
